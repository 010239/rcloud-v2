 ...
 *  3DES算法java实现
 * 
 *  @author lidonghui  DateTime 2014年7月9日 
 *
 */
public class Algorithm3DES {
	
	private static Logger logger = Logger.getLogger(Algorithm3DES.class);
	...
	public static void encryptMode(AlgorithmData data)throws Exception{
		try{
			...
			Assert.hasText(src, "需要加密的明文不能为空");
			SecretKey key = null;
	...
	public static void decryptMode(AlgorithmData data){
		try{
			...
			Assert.notNull(keystr,"密钥不能为空");
			Assert.notNull(srcstr,"需要解密的密文不能为空");
...
//		 System.out.println("加密密钥:"+data.getKey());
