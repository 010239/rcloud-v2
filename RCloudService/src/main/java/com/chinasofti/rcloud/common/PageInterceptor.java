...
public class PageInterceptor implements Interceptor {
	...
	private static int PAGINATION_OFFSET = Integer.parseInt(resource.getString("pagination.offset").trim());
...
//						page);
	 ...
	 * @throws Exception 
	 */
	private void validatePageParam(PageParameter page) throws Exception {
		int maxOffset = PAGINATION_OFFSET;
		if (maxOffset < 1000) {
			throw new Exception("分页最大偏移量小于1000，请联系管理员修改配置文件！");
		}
		int offset = (page.getCurrentPage() - 1) * page.getPageSize();
		if (offset > maxOffset) {
			offset = (maxOffset / page.getPageSize()) * page.getPageSize();
			page.setCurrentPage(maxOffset / page.getPageSize() + 1);
		}
	}
